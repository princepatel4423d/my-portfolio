...
const ExperienceList = () => {
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto p-4 text-center text-red-600 bg-red-100 rounded">
  ...
  return (
    <div className="space-y-8">
      ...
      {experiences.length === 0 ? (
        <div className="max-w-4xl mx-auto p-6 text-center text-gray-600 font-semibold">
          ...
          {experiences.map((exp) => (
            ...
            >
              <div className="p-6 flex flex-col flex-grow">
                ...
                {exp.description && (
                  <p className="text-gray-700 mb-3 max-w-full whitespace-pre-line line-clamp-2">
                    {exp.description}
                  </p>
                )}
                {exp.highlights?.length > 0 && (
                  <ul className="list-disc list-inside mt-auto text-gray-600">
                ...
                {exp.techStack?.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {exp.techStack.map((tech, idx) => (
                      <span
                        key={idx}
                        className="bg-blue-100 text-blue-800 rounded-full px-3 py-1 text-xs font-semibold whitespace-nowrap"
                      >
                        {tech}
                      </span>
                    ))}
